...
- **v3 API:** `TGravatar4DV3` in `src\Gravatar4D.pas` provides SHA256 avatar URLs/images, profile JSON retrieval/parsing (`TGravatarProfile`), and QR Code image download helpers.
...
- **v3 URL safety:** profile identifier path segments are encoded and QR query values (`type`) are parameter-encoded; preserve this behavior when extending v3 request builders.
- **v3 JSON parsing:** profile fields like `section_visibility` and `verified_accounts` may be null; check `TJSONValue` type (`is TJSONObject` / `is TJSONArray`) before casting.
